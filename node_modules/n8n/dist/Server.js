...
class Server extends AbstractServer_1.AbstractServer {
    constructor() {
        ...
        this.frontendSettings = {
            ...
            userManagement: {
                quota: typedi_1.Container.get(License_1.License).getUsersLimit(),
                showSetupOnFirstLoad: config_1.default.getEnv('userManagement.isInstanceOwnerSetUp') === false,
    ...
    async start() {
        ...
        await super.start();
        n8n_workflow_1.LoggerProxy.debug(`Server ID: ${this.uniqueInstanceId}`);
    ...
    getSettingsForFrontend() {
        ...
        Object.assign(this.frontendSettings.enterprise, {
            ...
            sourceControl: (0, sourceControlHelper_ee_1.isSourceControlLicensed)(),
            showNonProdBanner: typedi_1.Container.get(License_1.License).isFeatureEnabled(constants_1.LICENSE_FEATURES.SHOW_NON_PROD_BANNER),
            debugInEditor: (0, executionHelpers_1.isDebugInEditorLicensed)(),
        });
    ...
    async registerControllers(ignoredEndpoints) {
        const { app, externalHooks, activeWorkflowRunner, nodeTypes } = this;
        const repositories = Db.collections;
        (0, middlewares_1.setupAuthMiddlewares)(app, ignoredEndpoints, this.restEndpoint);
        ...
        const controllers = [
            ...
            new controllers_1.OwnerController({
                config: config_1.default,
                internalHooks,
                repositories,
                logger,
            }),
            new controllers_1.MeController({
                externalHooks,
                internalHooks,
                logger,
            }),
            ...
            typedi_1.Container.get(controllers_1.TagsController),
            ...
            typedi_1.Container.get(saml_controller_ee_1.SamlController),
            typedi_1.Container.get(sourceControl_controller_ee_1.SourceControlController),
            typedi_1.Container.get(controllers_1.WorkflowStatisticsController),
        ];
        if ((0, helpers_1.isLdapEnabled)()) {
        ...
        if (constants_1.inE2ETests) {
            ...
            controllers.push(typedi_1.Container.get(E2EController));
        }
    ...
    async configure() {
        ...
        try {
            ...
            n8n_workflow_1.LoggerProxy.warn(`Source Control initialization failed: ${error.message}`);
        }
        ...
        this.app.get(`/${this.restEndpoint}/get-mapping-fields`, ResponseHelper.send(async (req, res) => {
            const nodeTypeAndVersion = (0, n8n_workflow_1.jsonParse)(req.query.nodeTypeAndVersion);
            const { path, methodName } = req.query;
            if (!req.query.currentNodeParameters) {
                throw new ResponseHelper.BadRequestError('Parameter currentNodeParameters is required.');
            ...
            const loadMappingOptionsInstance = new n8n_core_1.LoadMappingOptions(nodeTypeAndVersion, this.nodeTypes, path, currentNodeParameters, credentials);
            const additionalData = await WorkflowExecuteAdditionalData.getBase(req.user.id, currentNodeParameters);
            const fields = await loadMappingOptionsInstance.getOptionsViaMethodName(methodName, additionalData);
            return fields;
        }));
        ...
        this.app.get(`/${this.restEndpoint}/executions-current`, ResponseHelper.send(async (req) => {
            if (config_1.default.getEnv('executions.mode') === 'queue') {
                ...
                const executions = await typedi_1.Container.get(repositories_1.ExecutionRepository).findMultipleExecutions(findOptions);
        ...
        this.app.post(`/${this.restEndpoint}/executions-current/:id/stop`, ResponseHelper.send(async (req) => {
            ...
            const fullExecutionData = await typedi_1.Container.get(repositories_1.ExecutionRepository).findSingleExecution(executionId, {
                where: {
        ...
        if (!config_1.default.getEnv('endpoints.disableUi')) {
            ...
            const serveIcons = async (req, res) => {
                ...
                if (loader) {
                    ...
                    if ((0, path_1.relative)(loader.directory, filePath).includes('..')) {
