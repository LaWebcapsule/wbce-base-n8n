...
function getSqliteDbFileSize() {
    const filename = path_1.default.resolve(n8n_core_1.UserSettings.getUserN8nFolderPath(), config_1.default.getEnv('database.sqlite.database'));
...
const pruneExecutionsData = async ({ queryRunner, tablePrefix, logger }) => {
    if (migrationsPruningEnabled) {
        ...
        const counting = (await queryRunner.query(`select count(id) as rows from "${tablePrefix}execution_entity";`));
