...
class PermissionChecker {
    static async check(workflow, userId) {
        ...
        if (!(0, UserManagementHelper_1.isSharingEnabled)()) {
            const role = await typedi_1.default.get(role_service_1.RoleService).findCredentialOwnerRole();
    ...
    static async checkSubworkflowExecutePolicy(subworkflow, userId, parentWorkflowId) {
        ...
        const subworkflowOwner = await typedi_1.default.get(ownership_service_1.OwnershipService).getWorkflowOwnerCached(subworkflow.id);
        ...
        if (policy === 'workflowsFromSameOwner') {
            const user = await typedi_1.default.get(user_service_1.UserService).findOne({ where: { id: userId } });
            if (!user) {
