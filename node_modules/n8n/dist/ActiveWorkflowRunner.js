...
let ActiveWorkflowRunner = exports.ActiveWorkflowRunner = class ActiveWorkflowRunner {
    constructor(activeExecutions, externalHooks, nodeTypes, webhookService) {
        ...
        if (workflowsData.length !== 0) {
            ...
            for (const workflowData of workflowsData) {
                ...
                try {
                    ...
                    n8n_workflow_1.LoggerProxy.info('     => ERROR: Workflow could not be activated on first try, keep on trying if not an auth issue');
    ...
    async executeWebhook(request, response) {
        ...
        return new Promise((resolve, reject) => {
            ...
            void WebhookHelpers.executeWebhook(workflow, webhookData, workflowData, workflowStartNode, executionMode, undefined, undefined, undefined, request, response, (error, data) => {
                if (error !== null) {
                    return reject(error);
                }
                resolve(data);
    ...
    async addWorkflowWebhooks(workflow, additionalData, mode, activation) {
        ...
        for (const webhookData of webhooks) {
            ...
            try {
                ...
                await workflow.createWebhookIfNotExists(webhookData, n8n_core_1.NodeExecuteFunctions, mode, activation, false);
            }
            catch (error) {
                if (activation === 'init' &&
                ...
                try {
                    ...
                    n8n_workflow_1.LoggerProxy.error(`Could not remove webhooks of workflow "${workflow.id}" because of error: "${error1.message}"`);
                }
    ...
    async removeWorkflowWebhooks(workflowId) {
        ...
        const webhooks = WebhookHelpers.getWorkflowWebhooks(workflow, additionalData, undefined, true);
        for (const webhookData of webhooks) {
            await workflow.deleteWebhook(webhookData, n8n_core_1.NodeExecuteFunctions, mode, 'update', false);
        }
        await WorkflowHelpers.saveStaticData(workflow);
        ...
        return (workflow, node) => {
            ...
            returnFunctions.__emit = (data, responsePromise, donePromise) => {
                ...
                if (donePromise) {
                    ...
                    void executePromise.catch(n8n_workflow_1.LoggerProxy.error);
                }
            };
            returnFunctions.__emitError = (error) => {
                void (0, GenericHelpers_1.createErrorExecution)(error, node, workflowData, workflow, mode).then(() => {
    ...
    getExecuteTriggerFunctions(workflowData, additionalData, mode, activation) {
        return (workflow, node) => {
            ...
            returnFunctions.emitError = (error) => {
