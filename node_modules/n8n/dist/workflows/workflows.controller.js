...
exports.workflowsController.post('/', ResponseHelper.send(async (req) => {
    ...
    if ((tagIds === null || tagIds === void 0 ? void 0 : tagIds.length) && !config_1.default.getEnv('workflowTagsDisabled')) {
        newWorkflow.tags = await typedi_1.Container.get(tag_service_1.TagService).findMany({
            select: ['id', 'name'],
            where: {
    ...
    await Db.transaction(async (transactionManager) => {
        ...
        const role = await typedi_1.Container.get(role_service_1.RoleService).findWorkflowOwnerRole();
    ...
    if (tagIds && !config_1.default.getEnv('workflowTagsDisabled') && savedWorkflow.tags) {
        savedWorkflow.tags = typedi_1.Container.get(tag_service_1.TagService).sortByRequestOrder(savedWorkflow.tags, {
            requestOrder: tagIds,
        });
    }
...
exports.workflowsController.get('/', middlewares_1.listQueryMiddleware, async (req, res) => {
    try {
        const sharedWorkflowIds = await WorkflowHelpers.getSharedWorkflowIds(req.user, ['owner']);
        const { workflows: data, count } = await workflows_services_1.WorkflowsService.getMany(sharedWorkflowIds, req.listQueryOptions);
        res.json({ count, data });
    }
    catch (maybeError) {
...
exports.workflowsController.get('/from-url', ResponseHelper.send(async (req) => {
    ...
    if ((workflowData === null || workflowData === void 0 ? void 0 : workflowData.nodes) === undefined ||
...
exports.workflowsController.get('/:id(\\w+)', ResponseHelper.send(async (req) => {
...
exports.workflowsController.patch('/:id(\\w+)', ResponseHelper.send(async (req) => {
    const { id: workflowId } = req.params;
...
exports.workflowsController.delete('/:id(\\w+)', ResponseHelper.send(async (req) => {
    const { id: workflowId } = req.params;
