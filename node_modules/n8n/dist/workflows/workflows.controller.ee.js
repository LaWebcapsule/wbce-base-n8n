...
exports.EEWorkflowController.get('/', middlewares_1.listQueryMiddleware, async (req, res) => {
    try {
        const sharedWorkflowIds = await WorkflowHelpers.getSharedWorkflowIds(req.user);
        const { workflows: data, count } = await workflows_services_ee_1.EEWorkflowsService.getMany(sharedWorkflowIds, req.listQueryOptions);
        res.json({ count, data });
    }
    catch (maybeError) {
...
exports.EEWorkflowController.patch('/:id(\\w+)', ResponseHelper.send(async (req) => {
    const { id: workflowId } = req.params;
